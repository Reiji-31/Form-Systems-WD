...
function showStep(index) {
    // Only target form cards (ignores the success/database screens)
...
function isStepValid() {
    ...
    // This checks all inputs, selects, and textareas in the current card.
    // If any required field is empty, it triggers the browser tooltip and stops the user.
    var inputs = card.querySelectorAll("input, select, textarea");
    for (let input of inputs) {
        if (!input.checkValidity()) {
            input.reportValidity();
            return false; // Stop the function, prevent moving to the next step
        }
    }
    // --------------------------------------
...
function nextStep() {
    ...
    if (currentStep < formCards.length - 1) showStep(currentStep + 1);
}
...
function goToStep(index) {
    if (index > currentStep && !isStepValid()) return;
    showStep(index);
}
...
function buildSummary() {
    ...
    if (getVal('form_for') === 'Someone Else') {
        ...
        addRow(tbody, 'Profile', 'Respondent Name', getVal('respondent_name'))
    }
    addRow(tbody, 'Profile', 'Name', getVal('full_name'));
...
var res_name = document.querySelector('[name="respondent_name"]')

function toggleForSomeone() {
    if (!formFor || (!relationship & !res_name)) return;
    if (formFor.value === "Myself") {
        relationship.value = "";
        relationship.disabled = true;
        relationship.classList.add("disabled-field");
        res_name.value = "";
        res_name.disabled = true;
        res_name.classList.add("disabled-field");
    } else {
        res_name.disabled = false;
        res_name.classList.remove("disabled-field");
...
function finalSubmit() {
    ...
    const entry = {
        ...
        res_name: getVal('respondent_name'),
...
function viewDatabase() {
    ...
    const sectionsHTML = [
        ...
        tableSection('sec-vax', 2, 'Vaccination Records', 
            [...baseHeaders, 'COVID Status', 'Annual Vaccines', 'Vax Document'],
            S.map(e => baseRow(e) + `<td>${e.vax_status}</td><td>${e.annual_vax}</td><td>${e.vax_doc}</td>`)),
